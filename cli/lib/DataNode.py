...
class DataNode:
    ...
    def __init__(self, dn_config, isNew = False):
        '''
        dn_config: a dictionary with arguments:
                id_n: id number
                dn_dir : datanode directory
                block_size : number of charecters stored in 1 block
                dn_size: number of blocks in a datanode
                dn_log_path : path to datanode log files
        
        '''
        
        try:
            attr = su.DN_attr(dn_config)
            
        except:
            raise ValueError("Poor attribute format")
            
        if not su.DN_check(attr,isNew):
            raise ValueError("Poor attributes for datanode construction")
    ...
    def get_status(self):
        ...
        alloc_arr = [i.strip().split(',')[2] for i in alloc_f.readlines()]
        alloc_f.close()

        log_f_name = attr.dn_log_path + su.fi_delim + 'log_%s.txt'%(attr.id)
        su.dn_status_log(log_f_name)

        return alloc_arr.count('0')
    ...
    def free_blks(self,dfs_file):
        ...
        log_f_name = attr.dn_log_path + su.fi_delim + 'log_%s.txt'%(attr.id)
        su.dn_free_log(log_f_name,dfs_file)
