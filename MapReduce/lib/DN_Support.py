...
class DN_attr:
    def __init__(self,dn_config):
        ...
        self.id = dn_config['id_n']
        self.dn_dir = dn_config['dn_dir']
        self.block_size = dn_config['block_size']
        self.dn_size = dn_config['dn_size']
        self.dn_log_path = dn_config['dn_log_path']
...
def dn_free_log(log_f_name,dfs_file):
    st = '%s File removed\n' %(dfs_file)
    log_f = open(log_f_name,'a')
    log_f.write(st)
    log_f.close()
...
def dn_status_log(log_f_name):

    st = 'Status update requested at %s\n' %(datetime.now().strftime("%H:%M:%S"))
    log_f = open(log_f_name,'a')
    log_f.write(st)
    log_f.close()